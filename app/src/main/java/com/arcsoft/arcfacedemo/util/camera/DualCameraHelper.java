...
import java.util.ArrayList;//相比CameraHelper增加动态数组类
...
public class DualCameraHelper implements Camera.PreviewCallback {
    private static List<Camera.Size> rgbSupportedPreviewSizes;//保存可见光摄像头支持的预览大小集合
    private static List<Camera.Size> irSupportedPreviewSizes;//保存红外摄像头支持的预览大小集合
    ...
    //用于获取双目摄像头的公共预览大小
    //基于双目摄像头的活体识别，要保证在相同预览大小状态下进行比对
    private List<Camera.Size> getCommonSupportedPreviewSize() {
         ...
         * 注意，这里认为前置摄像有是红外摄像头，后置摄像头是可见光摄像头
         */
    ...
    //相比CameraHelper增加了获取公共预览大小步骤，其他和CameraHelper一样
    public void start() {
        synchronized (this) {
            if (mCamera != null) {
                return;
            }
            List<Camera.Size> supportedPreviewSize = getCommonSupportedPreviewSize();//相比CameraHelper增加了获取公共预览大小步骤
            ...
            try {
                ...
                Log.d(TAG, "start: actual previewSize.width = " + previewSize.width +
     ...
     * 和CameraHelper基本一样，但是红外活体识别对分辨率有要求，要大于720p
     */
...
//            start();
    ...
    public static final class Builder {
        ...
        // 和CameraHelper一样，创建DualCameraHelper实例
